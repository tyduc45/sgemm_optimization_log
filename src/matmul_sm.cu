...
void sm_matmul_entry(const float* A, const float* B, float* C,
    ...
    sm_matmul<<<grid, block>>>(A, B, C, M, N, K, 1.0f, 0.0f);
    CHECK_CUDA(cudaGetLastError()); CHECK_CUDA(cudaDeviceSynchronize());
    if (!verify_matmul_with_cublas("shared-memory", A, B, C, M, N, K)) std::exit(EXIT_FAILURE);
    for (int i = 0; i < WARMUP_ITERS; ++i) sm_matmul<<<grid, block>>>(A, B, C, M, N, K, 1.0f, 0.0f);
    CHECK_CUDA(cudaGetLastError()); CHECK_CUDA(cudaDeviceSynchronize());
    timer.tic();
    for (int i = 0; i < BENCH_ITERS; ++i) sm_matmul<<<grid, block>>>(A, B, C, M, N, K, 1.0f, 0.0f);
